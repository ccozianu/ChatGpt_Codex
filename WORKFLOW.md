...
1. Start each session by reading the repository brief and its final handoff
   section.
2. Work from the active task list, not from stale conversation memory.
3. Keep each cycle narrow enough that the user can validate the result.
4. When the user validates something manually, update the handoff so the same
   task is not picked up again.
5. When an issue disappears or is deferred, remove it from the active task list
   and preserve the symptoms, logs, and reasoning in the completed-task archive.
6. Commit coherent units of work when asked, or at natural save points when the
...
section. The target project's `AGENTS.md` should instruct agents to read that
brief first, then any target-specific handoff notes. Retired debugging details
or important decisions should go under that project's `implementation-notes/`
folder. Closed task records should go under
`implementation-notes/completed-tasks/`.
